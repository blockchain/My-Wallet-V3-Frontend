describe('walletServices', () => {
  ...
  describe('mobile', () => {
    ...
    it('should allow change', inject(function (Wallet) {
      ...
      let newNumber = {country: "+31", number: "0100000000"};
      Wallet.changeMobile(newNumber, (function (){}),(function (){}));
    ...
    it('can be verified', inject(function (Wallet) {
      ...
      Wallet.verifyMobile("12345", (function (){}),(function (){}));
  ...
  describe('addAddressOrPrivateKey()', () => {
    beforeEach(function () {
      ...
      return Wallet.my.wallet.importLegacyAddress = function (privateKey, label, getPassword, bip38Password) {
        if (privateKey === "BIP38 key") {
          if (bip38Password === "5678") {
            return {
              then(success) { return success("some address"); }
            };
          } else {
            return {
              then(success, error) { return error("needsBip38"); }
            };
          }
        } else {
          let address = privateKey.replace("private_key_for_","");
          MyWalletStore.addLegacyAddress(address, privateKey, 200000000);
          return {
            then(success) { return success("address"); }
          };
        }
    ...
    it('should recoginize an address as such', () => {
      ...
      let success = address => expect(address.address).toBe("valid_address");

      return Wallet.addAddressOrPrivateKey("valid_address", null, success, null);
    });
    ...
    it('should derive the address corresponding to a private key', () => {
      ...
      return Wallet.addAddressOrPrivateKey("private_key_for_valid_address", null, success, null);
    });


    it('should complain if nothing is entered', () => {
      let success = () => expect(false).toBe(true);

      let error = errors => expect(errors.invalidInput).toBeDefined();

      return Wallet.addAddressOrPrivateKey("", null, success, error);
    });
    ...
    it('should complain if private key already exists', () => {
      ...
      let error = (errors, address) => expect(errors.addressPresentInWallet).toBeDefined();

      return address = Wallet.addAddressOrPrivateKey("private_key_for_some_legacy_address", null, success, error);
    });

    it('should complain if a watch-only address already exists', () => {
      let success = () => expect(false).toBe(true);

      let error = errors => expect(errors.addressPresentInWallet).toBeDefined();

      return Wallet.addAddressOrPrivateKey("some_legacy_watch_only_address", null, success, error);
    });
    ...
    it('should add private key to existing watch-only address', () => {
      ...
      let error = () => expect(false).toBe(true);

      return Wallet.addAddressOrPrivateKey("private_key_for_some_legacy_watch_only_address", null, success, error);
    });
  ...
  describe('notifications', () =>
    describe('on_tx', () => {
      ...
      it('should not display a message if the user spent bitcoin', () => {
        spyOn(Wallet.my.wallet.txList, 'transactions').and.callFake(() => [{ result: -1, txType: 'sent' }]);
      ...
      it('should not display a message if the user moved bitcoin between accounts', () => {
        spyOn(Wallet.my.wallet.txList, 'transactions').and.callFake(() => [{ result: 0, txType: 'transferred' }]);
  ...
  describe('legacyAddresses()', () => {
    ...
    it('should be an empty array if not logged in', () => {
      Wallet.status.isLoggedIn = false;
      let addrs = Wallet.legacyAddresses();
