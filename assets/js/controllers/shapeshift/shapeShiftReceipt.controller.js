...
function ShapeShiftReceiptController ($scope, Alerts, Env, localStorageService) {
  ...
  $scope.onClose = () => {
    ...
    if (shouldClose) { $scope.tabs.select('ORDER_HISTORY'); $scope.vm.goTo('create'); }
    else Alerts.surveyCloseConfirm(survey, links, 1).then(() => { $scope.tabs.select('ORDER_HISTORY'); $scope.vm.goTo('create'); });
  };
}
